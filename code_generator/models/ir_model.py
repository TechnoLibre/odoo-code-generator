...
class IrModelServerConstrain(models.Model):
    ...
    constrained = fields.Char(
        string="Constrained", help="Constrained fields, ej: name, age", required=True
    )
    ...
    def _check_constrained(self):
        if self.constrained:
            ...
            if list(
                filter(
                    lambda e: e.strip() not in self.env[self.m2o_ir_model.model]._fields.keys(),
                    splitted,
                )
            ):
                raise ValidationError(CONSTRAINEDLS)

    txt_code = fields.Text(string="Code", help="Code to execute", required=True)
    ...
    def _check_txt_code(self):
        if self.txt_code:
            ...
            common_4constrains(
                self.env[self.m2o_ir_model.model], self.txt_code, SYNTAXERRORMSG % constrain_detail
            )
...
class IrModel(models.Model):
    ...
    o2m_constraints = fields.One2many(
        "ir.model.constraint", "model", domain=[("type", "=", "u"), ("message", "!=", None)]
    )

    m2o_inherit_py_class = fields.Many2one(
        "code.generator.pyclass", string="Python Class", help="Python Class", ondelete="cascade"
    )

    m2o_inherit_model = fields.Many2one(
        "ir.model", string="Inherit Model", help="Inherit Model", ondelete="cascade"
    )
    ...
    nomenclator = fields.Boolean(
        string="Nomenclator?", help="Set this if you want this model as a nomenclator"
    )
...
class IrModelFields(models.Model):
    ...
    is_show_whitelist_list_view = fields.Boolean(
        ...
        help="If a field in model is in whitelist, all is not will be hide. " "View list only.",
    )
    ...
    code_generator_compute = fields.Char(
        string="Compute Code Generator", help="Compute method to code_generator_writer."
    )
    ...
    def create(self, vals):
        ...
        if vals.get("state", "manual") == "manual":
            ...
            if (
                vals.get("relation")
                and not self.env["ir.model"].search([("model", "=", vals["relation"])])
                and check_relation
            ):
            ...
            if vals["model"] in self.pool:
                ...
                self.pool.init_models(
                    self._cr, descendants, dict(self._context, update_custom_fields=True)
                )
    ...
    def unlink(self):
        ...
        for imc in self:
            try:
                ...
                _logger.warning(
                    "The registry entry associated with %s no longer exists" % imc.model.model
                )
