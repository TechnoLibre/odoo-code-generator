                ...
                with cw.indent():
                    ...
                    if post_init_hook_feature_code_generator:
                        with cw.indent():
                            ...
                            self.write_constraint(cw, constraint_ids, model_id)
        ...
        with cw.indent():
            ...
            if model_id and model_id.inherit_model_ids:
                if (
                    ...
                    lst_module_depend = sorted(
                        list(
                            ...
                            ).difference(set(lst_inherit_module_name_added))
                        )
                    )
                if lst_module_depend:
                    lst_inherit_module_name_added += lst_module_depend
                    ...
                    if len(lst_module_depend) == 1:
                        ...
                        cw.emit(
                            f"lst_depend_module = {str(lst_module_depend)}"
                        )
                        cw.emit(
                ...
                if lst_dependency:
                    ...
                    if len(lst_dependency) == 1:
                        ...
                        cw.emit(f"lst_depend_model = {str(lst_dependency)}")
                        cw.emit(
            ...
            if module.external_dependencies_id:
                ...
                for ext_depend in module.external_dependencies_id:
                    ...
                    cw.emit("value = {")
                    with cw.indent():
                        cw.emit(f'"module_id": code_generator_id.id,')
                        cw.emit(f'"depend": "{ext_depend.depend}",')
                        cw.emit(
                            '"application_type":'
