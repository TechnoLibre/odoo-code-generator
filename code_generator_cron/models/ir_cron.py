...
class IrCron(models.Model):
    ...
    force_use_datetime_installation = fields.Boolean(string="Force nextcall installation",
                                                     help="Force to adapt update nextcall at installation datetime.")

    ignore_threshold_time_upper = fields.Boolean(string="Keep all time",
                                                 help="True will respect specific time without adding relative time"
                                                      ", else ignore upper time then inverval_type.", default=True)
