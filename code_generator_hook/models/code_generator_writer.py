...
class ExtractorModule:
    def __init__(self, module, model_model):
        ...
        if not module.template_module_path_generated_extension:
            return
        relative_path_generated_module = module.template_module_path_generated_extension.replace("'", "").replace(", ",
                                                                                                                  "/")
        path_generated_module = os.path.normpath(os.path.join(module.path_sync_code, relative_path_generated_module,
                                                              module.template_module_name, '**', '*.py'))
        ...
        for children in f_ast.body:
            ...
            if type(children) == ast.ClassDef:
                ...
                for node in children.body:
                    if type(node) is ast.Assign and node.targets and node.targets[0].id == "_name" and \
                            node.value.s == self.model:
                        return children
    ...
    def search_field(self, class_model_ast):
        ...
        for node in class_model_ast.body:
            sequence += 1
            if type(node) is ast.Assign and type(node.value) is ast.Call and node.value.func.value.id == "fields":
                ...
                for keyword in node.value.keywords:
                    ...
                    if keyword_type is ast.Str:
                        ...
                        _logger.error(f"Cannot support keyword of variable {var_name} type {keyword_type} in filename "
                                      f"{self.py_filename}.")
...
class CodeGeneratorWriter(models.Model):
    ...
    def _write_sync_template(self, module, model_model, cw, var_model_model, lst_keep_f2exports, module_file_sync,
        ...
        for field_id in f2exports:
            ...
            with cw.block(before=f"{var_value_field_name} =", delim=('{', '}')):
                ...
                if field_id.ttype in ["many2one", "many2many", "one2many"]:
                    ...
                    field_selection = self.env[model_model].fields_get(field_id.name).get(field_id.name)
                    ...
                    if post_init_hook_feature_code_generator:
                        with cw.indent():
                            ...
                            enable_template_code_generator_demo = module.enable_template_code_generator_demo \
                                if module.name == "code_generator_demo" else False
                            if module.enable_template_code_generator_demo:
                                ...
                                if is_generator_demo:
                                    ...
                                    with cw.indent():
                                        with cw.block(before='value =', delim=('{', '}')):
                                            ...
                                            cw.emit("\"depend_id\": depend.id,")
                                            cw.emit("\"name\": depend.display_name,")
                                        cw.emit("env[\"code.generator.module.template.dependency\"].create(value)")
                                    cw.emit()
                                ...
                                for model_model in lst_model:
                                    ...
                                    if module.enable_sync_template:
                                        ...
                                        self._write_sync_template(module, model_model, cw, variable_model_model,
                                                                  lst_keep_f2exports, module_file_sync)
                                    else:
                            ...
                            if module.enable_template_wizard_view:
                                ...
                                with cw.indent():
                                    ...
                                    if module.enable_generate_portal:
                                        cw.emit(f"'enable_generate_portal': {module.enable_generate_portal},")
                                cw.emit("})")
                                cw.emit("")
