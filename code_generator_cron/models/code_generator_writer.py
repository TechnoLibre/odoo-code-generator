    ...
    def set_xml_data_file(self, module):
        ...
        for cron in module.o2m_model_cron:
            ...
            model_id_name = self._get_ir_model_data(
                cron.model_id, give_a_default=True
            )
                ...
                E.field(
                    {
                        ...
                        "ref": self._get_ir_model_data(
                            cron.user_id, give_a_default=True
                        ),
                    }
                ),
                E.field(
                    {"name": "interval_number"}, str(cron.interval_number)
                ),
            ...
            if cron.nextcall_template:
                ...
                if not cron.force_use_datetime_installation:
                    if cron.interval_type == "months":
                        ...
                        if not cron.interval_number % 12:
                            timestamp = (
                                f"%Y-{cron.nextcall.strftime('%m-%d %H:%M:%S')}"
                            )
                        else:
                            timestamp = (
                                f"%Y-%m-{cron.nextcall.strftime('%d %H:%M:%S')}"
                            )
                    elif cron.interval_type == "weeks":
                        timestamp = (
                            f"%Y-%m-%d {cron.nextcall.strftime('%H:%M:%S')}"
                        )
                        ...
                        timestamp = (
                            f"%Y-%m-%d %H:{cron.nextcall.strftime('%M:%S')}"
                        )
                    elif cron.interval_type == "minutes":
                        timestamp = (
                            f"%Y-%m-%d %H:%M:{cron.nextcall.strftime('%S')}"
                        )
                else:
                ...
                nextcall_field = (
                    "(datetime.now() +"
                    f" {str_timedelta}).strftime('{timestamp}')"
                )
                lst_field.append(
                    E.field({"name": "nextcall", "eval": nextcall_field})
                )
            ...
            record_xml = E.record(
                {"model": "ir.cron", "id": f"ir_cron_{i}"}, *lst_field
            )
