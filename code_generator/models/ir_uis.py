    ...
    def default_get(self, fields_list):
        ...
        if "parent_id" in result and result["parent_id"]:
            ...
            if parent_menu.name == "Menu Raíz Modulo A" and parent_menu.m2o_module:
                ...
                module_a_model_actionwindow = self.env["ir.actions.act_window"].search(
                    [("name", "=", "modulo.a.modelo.actionwindow")]
                )
                if module_a_model_actionwindow:
                    result["action"] = (
                        "ir.actions.act_window,%s" % module_a_model_actionwindow[0].id
                    )
...
class IrUiView(models.Model):
    ...
    m2o_model = fields.Many2one(
        "ir.model", string="Code generator Model", help="Model", ondelete="cascade"
    )
