    ...
    def _write_generated_template(self, module, model_model, cw):
        ...
        if module.enable_cron_template:
            ...
            with cw.block(before="value =", delim=('{', '}')):
                ...
                cw.emit('"nextcall_template": "(datetime.now() + timedelta(days=1)).strftime(\'%Y-%m-%d 03:00:00\')",')
    ...
    def set_xml_data_file(self, module):
        ...
        for cron in module.o2m_model_cron:
            ...
            lst_field = [
                ...
                E.field({"name": "user_id", "ref": self._get_ir_model_data(cron.user_id, give_a_default=True)}),
            ...
            if cron.nextcall_template:
                ...
                lst_field.append(E.field({"name": "nextcall"}, cron.nextcall.strftime('%Y-%m-%d %H:%M:%S')))
